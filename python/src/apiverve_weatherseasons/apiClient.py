...
class WeatherseasonsAPIClientError(Exception):
...
class ValidationError(WeatherseasonsAPIClientError):
...
class WeatherseasonsAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"year": {"type": "integer", "required": False, "min": 1900, "max": 2100, "default": 2025}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the WeatherseasonsAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises WeatherseasonsAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/weatherseasons'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise WeatherseasonsAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[WeatherseasonsAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns WeatherSeasonsData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises WeatherseasonsAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise WeatherseasonsAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise WeatherseasonsAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise WeatherseasonsAPIClientError(f"Request failed: {str(e)}")
